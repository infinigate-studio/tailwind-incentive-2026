...
export default function PodiumEntry({ entry, rank, highlighted, maxScore, icon, iconType }: Props) {
  const percent = maxScore > 0 ? Math.max(4, Math.round((entry.score / maxScore) * 100)) : 4;

  return (
    <li
      className={`row row--podium ${highlighted ? 'row--highlighted' : ''}`}
      aria-label={`Rank ${rank}: ${entry.name}, ${entry.score} points`}
    >
      <span className={`row__rank row__rank--${rank}`}>{rank}</span>
      <span className="row__name">{entry.name}</span>
      <div className="row__bar-track">
        <div
          className={`row__bar-fill row__bar-fill--${rank}`}
          style={{ width: `${percent}%` }}
        >
          <img
            src={icon}
